...
import com.bot.ncl.adapter.event.app.evt.CONV115902;
...
@Slf4j
@Component("CONV115902Lsnr")
@Scope("prototype")
public class CONV115902Lsnr extends BatchListenerCase<CONV115902> {
    ...
    private static final String CONVF_PATH_17X4115902 = "17X4115902";
    private static final String CONVF_PATH_CL005 = "CL005";
    private static final String CONVF_PATH_003 = "003";
    private static final String CONVF_PATH_FCL005115902 = "FCL005115902";
    private static final String CONVF_RPT = "RPT";
    private static final String CONVF_DATA = "DATA";
    private static final String CONVF_PATH_17X411590T = "17X411590T";
    ...
    private String writeFd115902TotPath;
    ...
    private List<String> fileFd115902Contents = new ArrayList<>();
    private List<String> fileFd115902TotContents = new ArrayList<>();

    @Override
    public void onApplicationEvent(CONV115902 event) {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "CONV115902Lsnr");
        this.beforRun(event);
    }

    @Override
    @SneakyThrows
    protected void run(CONV115902 event) {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "CONV115902Lsnr run()");
    ...
    private void init(CONV115902 event) {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "CONV115902Lsnr init()");
        // 讀批次日期檔
        this.event = event;
        ...
        String readFdDir =
                fileDir
                        ...
                        + PATH_SEPARATOR
                        + wkFdate;
        readFdPutfnPath = readFdDir + PATH_SEPARATOR + CONVF_PATH_17X4115902;
        ...
        String sourceFtpPath =
                "NCL"
                        ...
                        + CONVF_PATH_PUTFN
                        + File.separator
                        + CONVF_PATH_17X4115902; // 來源檔在FTP的位置
        File sourceFile = downloadFromSftp(sourceFtpPath, readFdDir);
        if (sourceFile != null) {
            readFdPutfnPath = getLocalPath(sourceFile);
        }
        //     COPY   DATA/GN/DWL/CL005/003/#DATE7/FCL005115902 AS
        //                  00069000
        //            DATA/CL/BH/PUTFN/#CLDATE/17X411590T;
        //                  00069050
        writeFd115902TotPath =
                fileDir
                        + CONVF_DATA
                        + PATH_SEPARATOR
                        + processDate
                        ...
                        + wkFdate
                        + PATH_SEPARATOR
                        + CONVF_PATH_17X411590T;
    }

    private void checkFdPutfnExist() {
        ApLogHelper.info(
                log, false, LogType.NORMAL.getCode(), "CONV115902Lsnr checkFdPutfnExist()");
        // 若FD-PUTFN檔案存在，執行115902-RTN，寫FD-115902、FD-115902-TOT(彙計總表之文字檔)
        // 若FD-PUTFN檔案不存在，執行115902-SUM-RTN，寫FD-115902-TOT(彙計總表之文字檔)
    ...
    private void readFdPutfnData() {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "CONV115902Lsnr readFdPutfnData()");
        // 循序讀取FD-PUTFN
    ...
    private void writeFd115902Rc1() {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "CONV115902Lsnr writeFd115902Rc1()");
        // 搬相關資料到115902-REC
        // 寫檔FD-115902(FIRST RECORD)
        ...
        sbRc1.append(formatUtil.pad9("918", 3));
        ...
        fileFd115902Contents.add(formatUtil.padX(sbRc1.toString(), 160));
    }

    private void writeFd115902Rc2() {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "CONV115902Lsnr writeFd115902Rc2()");
        // 寫檔FD-115902(DETAIL)
        ...
        fileFd115902Contents.add(formatUtil.padX(sbRc2.toString(), 160));
    }

    private void writeFd115902Rc3() {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "CONV115902Lsnr writeFd115902Rc3()");
        ...
        sbRc3.append(formatUtil.pad9("918", 3));
        ...
        fileFd115902Contents.add(formatUtil.padX(sbRc3.toString(), 160));
    }

    private void writeFd115902Tot() {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "CONV115902Lsnr writeFd115902Tot()");
        StringBuilder sbTot1 = new StringBuilder();
        sbTot1.append(formatUtil.padX("", 82));
        fileFd115902TotContents.add(formatUtil.padX(sbTot1.toString(), 82));
        ...
        sbTot5.append(
                formatUtil.padX(
                        processDate.substring(0, 3)
                                ...
                                + processDate.substring(5, 7),
                        9));
        fileFd115902TotContents.add(formatUtil.padX(sbTot5.toString(), 100));
        ...
        fileFd115902TotContents.add(formatUtil.padX(sbTot6.toString(), 100));
        ...
        fileFd115902TotContents.add(formatUtil.padX(sbTot7.toString(), 100));
        ...
        fileFd115902TotContents.add(formatUtil.padX(sbTot8.toString(), 100));
    }

    private void writeFile() {
        try {
            textFile.deleteFile(readFdPutfnPath);
            textFile.writeFileContent(readFdPutfnPath, fileFd115902Contents, CHARSET_UTF8);
    ...
    private void writeFileTot() {
        try {
            textFile.deleteFile(writeFd115902TotPath);
            textFile.writeFileContent(writeFd115902TotPath, fileFd115902TotContents, CHARSET_UTF8);
            upload(writeFd115902TotPath, "DATA", "PUTFN");
        } catch (LogicException e) {
            moveErrorResponse(e);
        }
    }
    ...
    private void forFsap() {
        Map<String, String> result =
                fsapBatchUtil.processFile(
                        ...
                        CONVF_PATH_17X4115902, // 來源檔案名稱(20碼長)
                        CONVF_PATH_17X4115902, // 目的檔案名稱(20碼長)
                        "1", // 同步/非同步記號(1碼長) 1：同步 2：非同步
                        "SRC", // 檔案類型(3碼長) 若未填寫預設為SRC(來源檔),若為處理完成的回覆結果檔請寫入”RTN”
                        "", // 對方FTP連線帳號
                        "", // 對方FTP連線密碼
                        "NCL_CONV115902", // 檔案設定代號 ex:CONVF001
                        "CL", // (產品代碼,系統別)，ex.”NCL”
