...
import com.bot.ncl.adapter.event.app.evt.C069;
...
@Component("C069Lsnr")
@Scope("prototype")
public class C069Lsnr extends BatchListenerCase<C069> {
    ...
    private static final String FILE_TMP_NAME = "TMPC069"; // tmp檔名
    private static final String FILE_NAME = "CL-BH-C069"; // 檔名
    ...
    private String sortTmpFilePath; // C069路徑
    private List<String> sortTmpFileContents; // Tmp檔案內容
    private List<String> fileC069Contents; // 檔案內容
    ...
    @Override
    public void onApplicationEvent(C069 event) {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "C069Lsnr");
        this.beforRun(event);
    }

    @Override
    @SneakyThrows
    protected void run(C069 event) {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "C069Lsnr run");
        init(event);
        sortFileContent();
        if (isBlankFile) {
            toWriteC069BlankFile();
        } else {
            toWriteC069File();
        }
        try {
            textFile.writeFileContent(outputFilePath, fileC069Contents, CHARSET_BIG5);
    ...
    // 初始化INITIAL-RTN.
    private void init(C069 event) {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "C069Lsnr init ....");
        //    MOVE              0     TO     WK-SUBCNT.
        //    MOVE              0     TO     WK-SUBAMT.
        //    MOVE              0     TO     WK-TOTCNT.
        //    MOVE              0     TO     WK-TOTAMT.
        ...
        fileC069Contents = new ArrayList<>();
    ...
    private void sortFileContent() {
        ApLogHelper.info(
                log, false, LogType.NORMAL.getCode(), "toWriteC069File sortFileContent ....");
        //        File inputFile = new File(inputFilePath);
        //     IF  KPUTH-CODE NOT = "121444" 只找代碼類別為121444的
        //     GO  TO  SORT-IN-RTN .
        //     IF     SD-USERDATA(1:7)  NOT  NUMERIC
        //       OR   SD-USERDATA(8:33) NOT = SPACES 只找
        //     GO  TO  SORT-IN-RTN .
        ...
        for (String detail : lines) {
            ...
            if (!parse.isNumeric(detail.substring(64, 71))
                    || !detail.substring(71, 104).trim().isEmpty()) {
                continue;
            }
            cnt++;
        ...
        // 3. SORT順序：SD-PBRNO主辦行, SD-DATE代收日, SD-RCPTID銷帳編號 由小到大排序
    ...
    private void toWriteC069File() {
        ApLogHelper.info(
                log, false, LogType.NORMAL.getCode(), "toWriteC069File toWriteC069File ....");
        ...
        for (String detail : lines) {
            ...
            ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "toWriteC069File FILE ....");
        }
    }

    // 計算小計總計DTL-SUM-RTN.
    private void sumTotal(BigDecimal sdAmt) {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "toWriteC069File sumTotal ....");
        //    ADD               1     TO     WK-SUBCNT,
        //                                   WK-TOTCNT.
        //    ADD       SD-AMT        TO     WK-SUBAMT,
        //                                   WK-TOTAMT.
    ...
    // 頁首RPT064-WTIT-RTN
    private void toC069FileHeader() {
        ApLogHelper.info(
                log, false, LogType.NORMAL.getCode(), "toWriteC069File toC069FileHeader ....");
        ...
        sb.append(formatUtil.padX(" 綜所稅退稅憑單報表－依第二付款行統計兌付 ", 44));
        ...
        fileC069Contents.add(sb.toString());
        //    MOVE     SPACES                  TO      REPORT-LINE.
        //    WRITE    REPORT-LINE          AFTER      1.
        fileC069Contents.add("");
    ...
    // RPT-DTL-RTN.
    private void toC069FileDetail(String detail, BigDecimal sdAmt) { // 明細內容
        ApLogHelper.info(
                log, false, LogType.NORMAL.getCode(), "toWriteC069File toC069FileDetail ....");
    ...
    // LAST-RECODE-RTN
    private void toC069FileFooter() { // 頁尾
        ApLogHelper.info(
                log, false, LogType.NORMAL.getCode(), "toWriteC069File toC069FileFooter ....");
    ...
    private void toWriteC069BlankFile() {
        ApLogHelper.info(
                log, false, LogType.NORMAL.getCode(), "toWriteC069File toWriteC069BlankFile ....");
