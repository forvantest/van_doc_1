...
import com.bot.ncl.adapter.event.app.evt.FTP00;
...
@Component("FTP00Lsnr")
@Scope("prototype")
public class FTP00Lsnr extends BatchListenerCase<FTP00> {
    ...
    private String acid;
    private String nflg;
    private String firstflg = "0";
    private String filename;

    //    @Async("batchThreadPoolTaskExecutor") // 如需平行處理請將此行註解拿掉
    @Override
    public void onApplicationEvent(FTP00 event) {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "FTP00Lsnr");
        this.beforRun(event);
    }

    @Override
    @SneakyThrows
    protected void run(FTP00 event) {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "FTP00Lsnr run()");
        if (!init(event)) {
            batchResponse();
            return;
        }
    ...
    private Boolean init(FTP00 event) {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "FTP00Lsnr init");
    ...
    private void _0000_main_rtn() {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "FTP00Lsnr _0000_main_rtn");
        // GENRTN
    ...
    private void fpt00() {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "FTP00Lsnr fpt00");
        ...
        String putfDir =
                fileDir
                        ...
                        + PATH_SEPARATOR
                        + wkTaskDate;
        String filePath_PUTF = putfDir + PATH_SEPARATOR + wkTaskPutfile;
        ...
        String putfnDir =
                fileDir
                        ...
                        + PATH_SEPARATOR
                        + wkTaskDate;
        String filePath_PUTFN = putfnDir + PATH_SEPARATOR + wkTaskPutfile;
    ...
    private void chgftp() {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "FTP00Lsnr chgftp");
        ...
        if ("730489".equals(code)) {
            ...
            String emptyDATA = fileDir + code + "_EMPTYOK";
    ...
    private void ftp() {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "FTP00Lsnr ftp");
    ...
    private void upload(String filePath) {
        try {
            ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "FTP00 upload = {}", filePath);
