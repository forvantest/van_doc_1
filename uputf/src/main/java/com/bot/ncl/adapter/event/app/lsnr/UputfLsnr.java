...
import com.bot.ncl.adapter.event.app.evt.Uputf;
...
@Component("UputfLsnr")
@Scope("prototype")
public class UputfLsnr extends BatchListenerCase<Uputf> {
    ...
    @Override
    public void onApplicationEvent(Uputf event) {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "UputfLsnr");
        this.beforRun(event);
    }

    @Override
    @SneakyThrows
    protected void run(Uputf event) {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "UputfLsnr run()");

        init(event);

        // 主程式0000-MAIN-RTN
        // PERFORM 0000-MAIN-RTN  THRU    0000-MAIN-EXIT.
        readUputhAndWritePutf();
        batchResponse(event);
    }

    private void init(Uputf event) {
        ...
        String uputhDir = fileDir + "DATA" + File.separator + processDate;
        fileUputhPath = uputhDir + File.separator + "UPUTH";
        ...
        String sourceFtpPath =
                "NCL"
                        ...
                        + File.separator
                        + "UPUTH"; // 來源檔在FTP的位置
        File sourceFile = downloadFromSftp(sourceFtpPath, uputhDir);
        if (sourceFile != null) {
    ...
    // 0000-MAIN-RTN
    private void readUputhAndWritePutf() {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "readUputhAndWritePutf()");
    ...
    private void wputf1() {
        ...
        // FD-UPUTH -> FD-PUTF，寫檔FD-PUTF(明細)，PUTF-CTL=21
        //     MOVE      SPACES            TO      PUTF-REC.
        //     MOVE      21                TO      PUTF-CTL.
        //     MOVE      UPUTH-CODE        TO      PUTF-CODE.
        //     MOVE      UPUTH-RCPTID      TO      PUTF-RCPTID.
        //     MOVE      UPUTH-DATE        TO      PUTF-DATE.
        //     MOVE      UPUTH-TIME        TO      PUTF-TIME.
        //     MOVE      UPUTH-AMT         TO      PUTF-AMT,PUTF-OLDAMT.
        //     MOVE      UPUTH-CLLBR       TO      PUTF-CLLBR.
        //     MOVE      UPUTH-LMTDATE     TO      PUTF-LMTDATE.
        //     MOVE      UPUTH-USERDATA    TO      PUTF-USERDATA.
        //     MOVE      UPUTH-SITDATE     TO      PUTF-SITDATE.
        //     MOVE      UPUTH-TXTYPE      TO      PUTF-TXTYPE.
        filePutf = null;
        filePutf = new FilePUTF();
        filePutf.setCtl("21");
        filePutf.setCode(fileUputh.getCode());
        filePutf.setRcptid(fileUputh.getRcptid().substring(0, 16));
        filePutf.setEntdy(fileUputh.getEntdy());
        filePutf.setTime(fileUputh.getTime());
        filePutf.setOldamt(fileUputh.getAmt());
        filePutf.setAmt(fileUputh.getAmt());
        filePutf.setCllbr(fileUputh.getCllbr());
        filePutf.setLmtdate(fileUputh.getLmtdate());
        filePutf.setUserdata(fileUputh.getUserdata());
        filePutf.setSitdate(fileUputh.getSitdate());
        filePutf.setTxtype(fileUputh.getTxtype());
        //     WRITE     PUTF-REC.
    ...
    private void changeFileName() {
        ...
        //     IF  ATTRIBUTE  RESIDENT  OF  FD-PUTF  =   VALUE(TRUE)
        //      OPEN  EXTEND  FD-PUTF
        //      MOVE  0       TO       WK-RTN
        //     ELSE
        //      OPEN  OUTPUT  FD-PUTF
    ...
    private void wputf2() {
        ...
        // 寫檔FD-PUTF(每一CODE寫一筆)，PUTF-CTL=22,代收類別、總金額、總筆數、挑檔起迄日
        //     MOVE SPACES             TO      PUTF-REC.
    ...
    private void wputfctl() {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "wputfctf2()");
        // FD-PUTF("DATA/CL/BH/PUTF/"+WK-FDATE+"/"+WK-PUTFILE)為新檔時
    ...
    private void findClmr() {
        ...
        //     MOVE      DB-CLMR-ULPUTDT    TO      PUTF-EDATE.
        fileSumPUTF.setEdate(cltmr == null ? "" : "" + cltmr.getUlputdt());
    ...
    private void batchResponse(Uputf event) {
