...
import com.bot.ncl.adapter.event.app.evt.CONV366009;
...
@Slf4j
@Component("CONV366009Lsnr")
@Scope("prototype")
public class CONV366009Lsnr extends BatchListenerCase<CONV366009> {
    ...
    private static final String CONVF_PATH_07Z1366009 = "07Z1366009";
    ...
    private String rcptid366009;
    private String yyymm366009;
    private String chkno366009;
    private String pbrno366009;
    private String date366009;
    private String ckflg366009;
    private String sdate366009;
    private String fAmt366009;
    private String bkno2_366009;
    private String fDate366009;
    private int putfDate;
    private int putfSitdate;
    private int wkTotcnt = 0;
    private BigDecimal putfAmt = ZERO;
    private BigDecimal amt366009 = ZERO;
    private BigDecimal wkTotAmt = ZERO;
    private List<String> fileFd366009Contents = new ArrayList<>();

    @Override
    public void onApplicationEvent(CONV366009 event) {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "CONV366009Lsnr");
        this.beforRun(event);
    }

    @Override
    @SneakyThrows
    protected void run(CONV366009 event) {
        ApLogHelper.info(log, false, LogType.NORMAL.getCode(), "CONV366009Lsnr run()");
    ...
    private void init(CONV366009 event) {
        // 讀批次日期檔
        this.event = event;
        ...
        String readFdDir =
                fileDir
                        ...
                        + PATH_SEPARATOR
                        + wkFdate;
        readFdPutfPath = readFdDir + PATH_SEPARATOR + CONVF_PATH_07Z1366009;
        ...
        String sourceFtpPath =
                "NCL"
                        ...
                        + "PUTF"
                        + File.separator
                        + CONVF_PATH_07Z1366009; // 來源檔在FTP的位置
    ...
    private void writeRc1() {
        ...
        sbRc1.append(formatUtil.padX("BPREMIUM", 8));
        sbRc1.append(formatUtil.padX("054004052428", 12));
        ...
        fileFd366009Contents.add(formatUtil.padX(sbRc1.toString(), 100));
    }
    ...
    private void valuateRc2() {
        ...
        fDate366009 = "0000000";
        rcptid366009 = putfRcptid;
        yyymm366009 = wkUserdata.substring(0, 4);
        chkno366009 = wkUserdata.substring(4, 6);
        amt366009 = putfAmt;
        fAmt366009 = "0000000";
        sdate366009 = "0" + parse.decimal2String(putfSitdate, 6, 0);
        date366009 = "0" + parse.decimal2String(date366009, 6, 0);
        checkSitdateYear();
        checkDateYear();
        bkno2_366009 = "004";
        pbrno366009 = putfCllbr;
        checkPutfDate();
    }

    private void checkSitdateYear() {
        if (putfSitdate < 990101) {
            sdate366009 = "1" + parse.decimal2String(putfSitdate, 6, 0);
        }
    }

    private void checkDateYear() {
        if (putfDate < 990101) {
            date366009 = "1" + parse.decimal2String(putfDate, 6, 0);
        }
    }

    private void checkPutfDate() {
        if (putfSitdate != putfDate) {
            ckflg366009 = "Y";
        } else {
            ckflg366009 = "";
        }
    ...
    private void writeRc2() {
        ...
        sbRc2.append(formatUtil.pad9(fDate366009, 7));
        sbRc2.append(formatUtil.padX(rcptid366009, 16));
        sbRc2.append(formatUtil.pad9(yyymm366009, 5));
        sbRc2.append(formatUtil.padX(chkno366009, 2));
        sbRc2.append(formatUtil.pad9(decimalFormat.format(amt366009), 12));
        sbRc2.append(formatUtil.padX(fAmt366009, 7));
        sbRc2.append(formatUtil.pad9(sdate366009, 7));
        sbRc2.append(formatUtil.pad9(date366009, 7));
        sbRc2.append(formatUtil.padX(bkno2_366009, 3));
        sbRc2.append(formatUtil.padX(pbrno366009, 4));
        sbRc2.append(formatUtil.padX("", 9));
        sbRc2.append(formatUtil.padX(ckflg366009, 1));
        sbRc2.append(formatUtil.padX("", 10));
        fileFd366009Contents.add(formatUtil.padX(sbRc2.toString(), 100));
    }
    ...
    private void writeSum() {
        ...
        fileFd366009Contents.add(formatUtil.padX(sbSum.toString(), 100));
    }

    private void writeFile() {
        try {
            textFile.deleteFile(readFdPutfPath);
            textFile.writeFileContent(readFdPutfPath, fileFd366009Contents, CHARSET_UTF8);
    ...
    private void forFsap() {
        Map<String, String> result =
                fsapBatchUtil.processFile(
                        ...
                        CONVF_PATH_07Z1366009, // 來源檔案名稱(20碼長)
                        CONVF_PATH_07Z1366009, // 目的檔案名稱(20碼長)
                        "2", // 同步/非同步記號(1碼長) 1：同步 2：非同步
                        "SRC", // 檔案類型(3碼長) 若未填寫預設為SRC(來源檔),若為處理完成的回覆結果檔請寫入”RTN”
                        "", // 對方FTP連線帳號
                        "", // 對方FTP連線密碼
                        "NCL_CONV366009", // 檔案設定代號 ex:CONVF001
                        "CL", // (產品代碼,系統別)，ex.”NCL”
