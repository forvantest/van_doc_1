            ...
            steps {
                script {
                    if (params.size()>0) {
                        ...
                        properties([
                            parameters([
                                choice(name: 'env', choices: ['dev', 'sit'], description: '部署環境。可參考 gradle/deploy/deploy.gradle'),
                                ...
                                booleanParam(name: 'is_offline', defaultValue: false, description: '是否離線包版 ?'),
            ...
            steps {
                script {
                    ...
                    if (params.is_offline) {
                        sh "./gradlew ${str_project}clean ${str_project}build --refresh-dependencies -x check --stacktrace --console=plain --offline"
                    } else {
                        sh "./gradlew ${str_project}clean ${str_project}build --refresh-dependencies -x check --stacktrace --console=plain"
                    }
        ...
        stage('Remote Start') {
            ...
            steps {
                script {
                    def str_project = "${gradle_project}" == "rootProject" ? ":" : "${gradle_project}".length() > 0 ? "${gradle_project}:" : ""
                    if (params.is_offline) {
                        sh "./gradlew ${str_project}remote_start -Denv=${params.env} -Dexclude_remotes=${params.exclude_remotes} -Dinclude_remotes=${params.include_remotes} --stacktrace --console=plain --offline"
                    } else {
                        sh "./gradlew ${str_project}remote_start -Denv=${params.env} -Dexclude_remotes=${params.exclude_remotes} -Dinclude_remotes=${params.include_remotes} --stacktrace --console=plain"
                    }
                }
            }
        }
